...
var secretNumber = (Math.floor(Math.random() * (9999 - 1000 + 1)) + 1000).toString();
var theme = document.getElementsByClassName('select');
theme[0].addEventListener('click',function(){
    console.log("selected");
    document.documentElement.style.setProperty('--themeWallpaper', 'url(basic.jpg)');
})
theme[1].addEventListener('click',function(){
    console.log("selected");
    document.documentElement.style.setProperty('--themeWallpaper', 'url(scotland.jpg)');
})
theme[2].addEventListener('click',function(){
    console.log("selected");
    document.documentElement.style.setProperty('--themeWallpaper', 'url(nfs.jpg)');
})
theme[3].addEventListener('click',function(){
    console.log("selected");
    document.documentElement.style.setProperty('--themeWallpaper', 'url(mountains.jpg)');
})
theme[4].addEventListener('click',function(){
    console.log("selected");
    document.documentElement.style.setProperty('--themeWallpaper', 'url(space.jpg)');
})
theme[5].addEventListener('click',function(){
    console.log("selected");
    document.documentElement.style.setProperty('--themeWallpaper', 'url(magic.jpg)');
})
theme[6].addEventListener('click',function(){
    console.log("selected");
    document.documentElement.style.setProperty('--themeWallpaper', 'url(fantasy.jpg)');
})
theme[7].addEventListener('click',function(){
    console.log("selected");
    document.documentElement.style.setProperty('--themeWallpaper', 'url(dolphins.jpg)');
})
theme[8].addEventListener('click',function(){
    console.log("selected");
    document.documentElement.style.setProperty('--themeWallpaper', 'url(dino.jpg)');
})
theme[9].addEventListener('click',function(){
    console.log("selected");
    document.documentElement.style.setProperty('--themeWallpaper', 'url(city.jpeg)');
})
...
function setCurrPlayerNameAndIcon(){
    ...
    if(currPlayerName.value!=''){
        ...
        // setting the icon
        document.getElementsByClassName('profileIcon')[0].innerText = currPlayerName.value.charAt(0);
    }
}
...
function numberProcessing(number){
    ...
    for(let i=0;i<number.length;i++){
        if(secretNumberCopy.charAt(i)==number.charAt(i)){
            ...
            secretNumberCopy = secretNumberCopy.replace(secretNumberCopy.charAt(i),"");
            bulls = bulls + 1;
    ...
    if(bulls!=4){
        for(let i=0;i<number.length;i++) {
            for(let j=0;j<secretNumberCopy.length;j++){
                if(number.charAt(j)==secretNumberCopy
                .charAt(i) && i!=j) {
                    secretNumberCopy = secretNumberCopy.replace(secretNumberCopy.charAt(j),"a");
                    cows++;
                    break;
                }
            }
    ...
    }else if(bulls==player.current.bestBulls && cows > player.current.bestCows){
        player.current.bestCows = cows;
        player.current.bestAttempt = numberCopy;
        player.current.bestGuess = bulls + "B" + cows + "C";
    }
...
function updateInfo(){
    var toUpdate = document.getElementById('dataContainer').getElementsByClassName('value');
...
function updateHistory(){
    var toUpdateNumber = document.getElementById('historyContainer').getElementsByClassName('type');
    var toAddLightEffect = document.getElementById('historyContainer').getElementsByClassName('line');
    var toUpdateScore = document.getElementById('historyContainer').getElementsByClassName('value');
    for(var i=0;i<12;i++){
...
function matchEnd(){
    ...
    player.totalAttempt = player.totalAttempt + player.current.currAttemptNumber;
    player.totalMatchs++;
    ...
    if(bulls==4){
        player.matchWon++;
        matchResult.innerText = 'Congratulations '+currPlayerName.value+' You Won '+' In Just '+player.current.currAttemptNumber + ' Attempts';
    }else{
        player.matchDraw++;
        matchResult.innerText = currPlayerName.value+' Final Score is '+bulls + ' Bulls and '+cows+' Cows';
    }
    updatePlayerInfo();
...
function updatePlayerInfo(){
    var statics = document.getElementById('statics').getElementsByClassName('value');
...
function newMatchStarted(){
    ...
    // for hint
    var i = 0;
    secretNumber = (Math.floor(Math.random() * (9999 - 1000 + 1)) + 1000).toString();
    var toUpdateNumber = document.getElementById('historyContainer').getElementsByClassName('type');
    var toUpdateScore = document.getElementById('historyContainer').getElementsByClassName('value');
    for(var i=0;i<13;i++){
